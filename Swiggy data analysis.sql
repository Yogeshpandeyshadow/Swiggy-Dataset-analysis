...
FROM [dbo].[SwiggyData1] A JOIN COUNTRY_REST B
ON A.[COUNTRY_NAME] = B.[COUNTRY_NAME]
...
--FINDING FROM WHICH CITY AND LOCALITY IN INDIA WHERE THE MAX RESTAURANTS ARE LISTED IN Swiggy
WITH CT1
AS
(
...
--TYPES OF FOODS ARE AVAILABLE IN INDIA WHERE THE MAX RESTAURANTS ARE LISTED IN Swiggy
WITH CT1
AS
(
SELECT [City],[Locality],COUNT([RestaurantID]) REST_COUNT
...
CT3 AS (
SELECT [Locality],[Cuisines] FROM [dbo].[SwiggyData1]
)
...
--MOST POPULAR FOOD IN INDIA WHERE THE MAX RESTAURANTS ARE LISTED IN Swiggy
CREATE VIEW VF 
AS
(
SELECT [COUNTRY_NAME],[City],[Locality],N.[Cuisines] FROM [dbo].[SwiggyData1]
...
--WHICH LOCALITIES IN INDIA HAS THE LOWEST RESTAURANTS LISTED IN Swiggy
WITH CT1 AS
(
SELECT [City],[Locality], COUNT([RestaurantID]) REST_COUNT
FROM [dbo].[SwiggyData1]
...
--HOW MANY RESTAURANTS OFFER TABLE BOOKING OPTION IN INDIA WHERE THE MAX RESTAURANTS ARE LISTED IN Swiggy
WITH CT1 AS (
SELECT [City],[Locality],COUNT([RestaurantID]) REST_COUNT
FROM [dbo].[SwiggyData1]
WHERE [COUNTRY_NAME] = 'INDIA'
